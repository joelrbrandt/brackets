...
define(function (require, exports, module) {
    ...
    */
    ...
    var nodeCount = 1;
    var nodes = []; // since all a node contains is the suffix_node, we just store it directly in an int array
    ...
    */
    ...
    var Edge = function (init_first_char_index, init_last_char_index, parent_node) {
    ...
    */
    ...
    Edge.prototype.insert = function () {
        if (!edges[this.start_node]) {
            edges[this.start_node] = {};
        }

        if (!edges[this.start_node][text[this.first_char_index]]) {
            edges[this.start_node][text[this.first_char_index]] = this;
        } else {
            throw new Error("tried to insert two edges with the same start node and character!");
        }
    };
    ...
    */
    ...
    Edge.prototype.remove = function () {
        var hash = edges[this.start_node];
        if (hash) {
            if (hash[text[this.first_char_index]]) {
                delete hash[text[this.first_char_index]];
            } else {
                throw new Error("tried to remove Edge from hash when something with this char index wasn't present!");
            }
        } else {
            throw new Error("Tried to remove Edge from hash when there wasn't a subhash with this start node!");
        }
    };
    ...
    */
    ...
    Edge.prototype.splitEdge = function (suf) {
        this.remove();
        var new_edge = new Edge(this.first_char_index,
                                this.first_char_index + suf.last_char_index - suf.first_char_index,
                                suf.origin_node);
        new_edge.insert();
        nodes[new_edge.end_node] = suf.origin_node;
        this.first_char_index += suf.last_char_index - suf.first_char_index + 1;
        this.start_node = new_edge.end_node;
        this.insert();
        return new_edge.end_node;
    };
    ...
    */
    ...
    var addPrefix = function (active, last_char_index) {
        var parent_node;
        var last_parent_node = -1;
        while (true) {
            var edge;
            parent_node = active.origin_node;
    ...
    */
            var new_edge = new Edge(last_char_index, text_length, parent_node);
            new_edge.insert();
            if (last_parent_node > 0) {
